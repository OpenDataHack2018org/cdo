...
static
void reg2d_search_do_cell_search (struct grid_search * search,
                                   struct grid * grid_data,
                                   struct dep_list * tgt_to_src_cells);
static
void reg2d_search_do_cell_search_single (struct grid_search * search,
                                          struct grid_cell grid_cell,
                                          unsigned * n_cells, unsigned * cells_size,
                                          unsigned ** cells);
static
void reg2d_search_do_point_search_c (struct grid_search * search,
                                      struct grid * grid_data,
                                      struct dep_list * tgt_to_src_cells);
static
void reg2d_search_do_point_search_c2 (struct grid_search * search,
                                       double * x_coordinates,
                                       double * y_coordinates, unsigned num_points,
                                       struct dep_list * tgt_to_src_cells);
static
void reg2d_search_do_point_search_p (struct grid_search * search,
                                      struct grid * grid_data,
                                      struct dep_list * target_to_src_points);
static
void reg2d_search_do_point_search_p2 (struct grid_search * search,
                                       double * x_coordinates,
                                       double * y_coordinates, unsigned num_points,
                                       struct dep_list * target_to_src_points);
static
void reg2d_search_do_point_search_p3 (struct grid_search * search,
                                       double * x_coordinates,
                                       double * y_coordinates, unsigned num_points,
                                       struct dep_list * target_to_src_points,
                                       struct points * points);
static
void reg2d_search_do_point_search_p4 (struct grid_search * search,
                                       double x_coordinate, double y_coordinate,
                                       unsigned * n_points, unsigned * points_size,
                                       unsigned ** points);

static void delete_reg2d_search(struct grid_search * search);
...
static struct grid_search_vtable reg2d_search_vtable =
{
   .do_cell_search        = reg2d_search_do_cell_search,
   .do_cell_search_single = reg2d_search_do_cell_search_single,
   .do_point_search_c     = reg2d_search_do_point_search_c,
   .do_point_search_c2    = reg2d_search_do_point_search_c2,
   .do_point_search_p     = reg2d_search_do_point_search_p,
   .do_point_search_p2    = reg2d_search_do_point_search_p2,
   .do_point_search_p3    = reg2d_search_do_point_search_p3,
   .do_point_search_p4    = reg2d_search_do_point_search_p4,
   .delete_grid_search    = delete_reg2d_search
};
...
static
void init_buckets(struct reg2d_search * reg2d_search, double grid_extent[2][2],
                  unsigned num_cells) {
   ...
   reg2d_search->bucket_grid_x_coords = coordinates_x;
   reg2d_search->bucket_grid_y_coords = coordinates_y;
...
//------------------------------------------------------------------
static
void search_on_bucket_grid (struct reg2d_search * reg2d_search, struct grid * grid_data,
      ...
      for (j = 0; j < num_corners; ++j) {
         ...
         if ((position_x[curr_cell_x_corner] != 0) &&
             (fabs(x_coords[curr_cell_x_corner] -
                   reg2d_search->bucket_grid_x_coords[position_x[curr_cell_x_corner]]) < tol)) {
         ...
         } else if ((position_x[curr_cell_x_corner] != reg2d_search->num_buckets[0] - 1) &&
             (fabs(x_coords[curr_cell_x_corner] -
                   reg2d_search->bucket_grid_x_coords[position_x[curr_cell_x_corner] + 1]) < tol)) {
         ...
         if ((position_y[curr_cell_y_corner] != 0) &&
             (fabs(y_coords[curr_cell_y_corner] -
                   reg2d_search->bucket_grid_y_coords[position_y[curr_cell_y_corner]]) < tol)) {
         ...
         } else if ((position_y[curr_cell_y_corner] != reg2d_search->num_buckets[1] - 1) &&
             (fabs(y_coords[curr_cell_y_corner] -
                   reg2d_search->bucket_grid_y_coords[position_y[curr_cell_y_corner]+1]) < tol)) {
...
static
void search_on_bucket_grid_single (struct reg2d_search * reg2d_search,
                                   struct grid_cell grid_cell,
   ...
   for (j = 0; j < grid_cell.num_corners; ++j) {
      ...
      if ((position_x[j] != 0) &&
          (fabs(grid_cell.coordinates_x[j] -
                reg2d_search->bucket_grid_x_coords[position_x[j]]) < tol)) {
      ...
      // if the current point falls onto the right edge of the bucket
      } else if ((position_x[j] != reg2d_search->num_buckets[0] - 1) &&
          (fabs(grid_cell.coordinates_x[j] -
                reg2d_search->bucket_grid_x_coords[position_x[j] + 1]) < tol)) {
      ...
      if ((position_y[j] != 0) &&
          (fabs(grid_cell.coordinates_y[j] -
                reg2d_search->bucket_grid_y_coords[position_y[j]]) < tol)) {
      ...
      // if the current point falls onto the upper edge of the bucket
      } else if ((position_y[j] != reg2d_search->num_buckets[1] - 1) &&
          (fabs(grid_cell.coordinates_y[j] -
                reg2d_search->bucket_grid_y_coords[position_y[j]+1]) < tol)) {
...
struct grid_search * reg2d_search_new (struct grid * grid_data) {
   ...
   struct reg2d_search * search;

   search = malloc(1 * sizeof(*search));

   search->vtable = &reg2d_search_vtable;
...
void reg2d_search_do_cell_search (struct grid_search * search,
   ...
   search_on_bucket_grid (reg2d_search, grid_data, &tgt_to_bucket);
   ...
   for (i = 0; i < num_tgt_grid_cells; ++i) {
      ...
      for (j = 0; j < tgt_to_bucket.num_deps_per_element[i]; ++j) {
         ...
         for (k = 0; k < reg2d_search->bucket_to_cell.num_deps_per_element[curr_bucket]; ++k) {
         ...
         } // (k = 0; k < reg2d_search->bucket_to_cell.num_deps_per_element[curr_bucket], ++k)
...
void reg2d_search_do_cell_search_single (struct grid_search * search,
   ...
   search_on_bucket_grid_single (reg2d_search, cell, &cell_to_bucket,
