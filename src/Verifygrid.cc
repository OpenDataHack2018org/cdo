...
/* The LLtoXYZ functions along with their comments was taken from clipping/geometry.h and adapted to long double. */
 ...
 */
static inline void LLtoXYZ_ld(double lon, double lat, long double p_out[]) {

  long double cos_lat = (long double) cos(lat);
  p_out[0] = (long double) cos_lat * cos(lon);
  p_out[1] = (long double) cos_lat * sin(lon);
  p_out[2] = (long double) sin(lat);
}
 ...
 * @param[out] p_out xyz coordinates
 */
static inline void LLtoXYZ_deg_ld(double lon, double lat, long double p_out[]) {
   LLtoXYZ_ld(lon*rad, lat*rad, p_out);
}

/* The following functions exist in double and long double versions. */ 
...
/* Quicksort is called with a pointer to the array to be sorted and an integer indicating its length. */
static
void quick_sort_ld(long double * array, size_t array_length)
{
  size_t i, j;
  long double temp;
  
  if (array_length < 2) return;
  long double p = array[array_length / 2];
  for (i = 0, j = array_length - 1;; i++, j--)
    {
      while (array[i] < p) i++;
      while (p < array[j]) j--;
  ...
  quick_sort_ld(array, i);
  quick_sort_ld(array + i, array_length - i);
}
...
/* Quicksort is called with a pointer to the array of center points to be sorted and an integer indicating its length. It sorts the array by its longitude coordinates */
static
void quick_sort_by_lon_ld(long double * array, size_t array_length)
{
  if ( array_length < 4 ) return;
  ...
  quick_sort_by_lon_ld(array, i);
  quick_sort_by_lon_ld(array + i, array_length - i);
}
...
/* This uses quicksort to sort the latitude coordinates in a subarray of all coordinates. */
static
void quick_sort_of_subarray_by_lat_ld(long double * array, size_t subarray_start, size_t subarray_end)
{
  size_t subarray_length = (subarray_end - subarray_start) / 2 + 1;     
  ...
  quick_sort_ld(subarray, subarray_length);
...
static
long double determinant_ld(long double matrix[3][3])
{  
  /* Calculates the determinant for a 3 x 3 matrix. */
...
static
void find_unit_normal_ld(long double a[3], long double b[3], long double c[3], long double *unit_normal)
{  
  /* Calculates the unit normal for a plane defined on three points a, b, c in Euclidean space. */

  long double matrix_for_x[3][3] = {{1, a[1], a[2]},
			       {1, b[1], b[2]},
			       {1, c[1], c[2]}};

  long double x = determinant_ld(matrix_for_x);

  long double matrix_for_y[3][3] = {{a[0], 1, a[2]},
			       {b[0], 1, b[2]},
			       {c[0], 1, c[2]}};
  
  long double y = determinant_ld(matrix_for_y);
  
  long double matrix_for_z[3][3] = {{a[0], a[1], 1},
			       {b[0], b[1], 1},
			       {c[0], c[1], 1}};

  long double z = determinant_ld(matrix_for_z);

  long double magnitude = sqrt(x * x + y * y + z * z);
...
int find_coordinate_to_ignore_ld(long double *cell_corners_xyz)
{
  /* Takes the first three corners/vertices of the cell and calculates the unit normal via determinants. */
      
  long double *pcorner_coordinates1 = &cell_corners_xyz[0];
  long double *pcorner_coordinates2 = &cell_corners_xyz[3];
  ...
  long double abs_x = fabs(surface_normal_of_the_cell[0]);
  long double abs_y = fabs(surface_normal_of_the_cell[1]);
  long double abs_z = fabs(surface_normal_of_the_cell[2]);
...
static
long double is_point_left_of_edge_ld(long double point_on_line_1[2], long double point_on_line_2[2], long double point[2])
{
  /* 
     Computes whether a point is left of the line through point_on_line_1 and point_on_line_2.
     This is part of the solution to the point in polygon problem.
...
int winding_numbers_algorithm_ld(long double cell_corners[], int number_corners, long double point[])
{  
  /* 
     Computes whether a point is inside the bounds of a cell. This is the solution to the point in polygon problem.
     Returns 0 if the point is outside, returns 1 if the point is inside the cell.
...
static
long double sign_ld(long double x)
{
  /* Is +1 if x is positive, -1 if x is negative and 0 if x is zero.*/
...
static
bool is_simple_polygon_convex_ld(long double cell_corners[], int number_corners)
{
   /* Tests in which direction the polygon winds when walking along its edges. Does so for all edges of the polygon. */

  long double direction = 0;
  ...
  for ( int i = 0; i < number_corners - 2; i++ )
    {
      ...
      if ( IS_NOT_EQUAL(sign_ld(direction), sign_ld(turns_to)) )
        {
          if ( IS_NOT_EQUAL(direction, 0) ) return false;
        }
      else
...
long double calculate_the_polygon_area_ld(long double cell_corners[], int number_corners)
{
  /* This algorithm is based on the calculation from Wolfram Mathworld Polygon Area. It results in the area of planar non-self-intersecting polygon. */
...
bool are_polygon_vertices_arranged_in_clockwise_order_ld(long double cell_area)
{
  bool status = false;
...
void verify_grid(int gridtype, size_t gridsize, int gridno, int ngrids, int ncorner, double *grid_center_lon, double *grid_center_lat, double *grid_corner_lon, double *grid_corner_lat)
{
  ...
  */
  ...
  /* 
     ...
     This is first done for the presumed center point of the cell and then for all the corners of the cell. LLtoXYZ_deg is defined in clipping/geometry.h 
  */
...
static
void verify_grid_ld(int gridtype, size_t gridsize, int gridno, int ngrids, int ncorner, double *grid_center_lon, double *grid_center_lat, double *grid_corner_lon, double *grid_corner_lat)
{
  /* 
     First, this function performs the following test:
  ...
  */
  ...
  quick_sort_by_lon_ld(center_point_array, gridsize * 2);
  ...
  */
  ...
  for ( size_t cell_no = 0; cell_no < gridsize; cell_no++ )
	...
	{	  
	  ...
	  LLtoXYZ_deg_ld(grid_corner_lon[cell_no * ncorner + corner_no], grid_corner_lat[cell_no * ncorner + corner_no], corner_coordinates);
	...
	for ( int j = i + 3; j < actual_number_of_corners * 3; j = j + 3 )
      ...
      int coordinate_to_ignore = find_coordinate_to_ignore_ld(cell_corners_xyz);
	...
	no_convex_cells += 1;
      ...
      long double polygon_area = calculate_the_polygon_area_ld(cell_corners_plane_projection, actual_number_of_corners + 1);
	...
	no_counterclockwise_cells +=1;
      ...
      int winding_number = winding_numbers_algorithm_ld(cell_corners_plane_projection, actual_number_of_corners + 1, center_point_plane_projection);
      ...
      if ( lgeo )
        {
          ...
          if ( operatorID == VERIFYGRID )
            verify_grid_ld(gridtype, gridsize, gridno, ngrids, ncorner, grid_center_lon, grid_center_lat, grid_corner_lon, grid_corner_lat);
