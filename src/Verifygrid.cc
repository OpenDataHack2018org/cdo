...
/* The LLtoXYZ functions along with their comments was taken from
 * clipping/geometry.h and adapted to long double. */
 ...
 */
static inline void
LLtoXYZ_ld(double lon, double lat, long double p_out[])
{
 ...
 */
static inline void
LLtoXYZ_deg_ld(double lon, double lat, long double p_out[])
{
...
/* Quicksort is called with a pointer to the array to be sorted and an integer
 ...
 * indicating its length. */
static void
quick_sort_ld(long double *array, size_t array_length)
{
  size_t i, j;
  long double temp;
 ...
 * coordinates */
static void
quick_sort_by_lon_ld(long double *array, size_t array_length)
{
  if (array_length < 4) return;
...
static long double
determinant_ld(long double matrix[3][3])
{
  /* Calculates the determinant for a 3 x 3 matrix. */
...
int
find_coordinate_to_ignore_ld(long double *cell_corners_xyz)
{
  /* Takes the first three corners/vertices of the cell and calculates the unit
   * normal via determinants. */
...
static bool
is_simple_polygon_convex_ld(long double cell_corners[], int number_corners)
{
  /* Tests in which direction the polygon winds when walking along its edges.
   * Does so for all edges of the polygon. */
...
long double
calculate_the_polygon_area_ld(long double cell_corners[], int number_corners)
{
  /* This algorithm is based on the calculation from Wolfram Mathworld Polygon
   * Area. It results in the area of planar non-self-intersecting polygon. */
...
bool
are_polygon_vertices_arranged_in_clockwise_order_ld(long double cell_area)
{
  bool status = false;
