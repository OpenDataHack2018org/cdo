...
static void
scale_vec(double scalefactor, size_t nvals, double *restrict values)
{
#ifdef _OPENMP
...
void
grid_copy_attributes(int gridID1, int gridID2)
{
                               ...
                               double *restrict grid_corner_lon, double dlon)
{
  ...
  if ( xsize == 1 || (grid_center_lon[xsize-1]-grid_center_lon[0]+dlon) < 359 )
    cdoAbort("Cannot calculate Xbounds for %d vals with dlon = %g", xsize,
  dlon);
  */
 ...
 *
 * Description:
 *   computes the number of points within the range lon_first->lon_last and the
 * zero based indexes ilon_first,ilon_last of the first and last point given the
 * number of points along a parallel (pl)
 *
 */
static void
grib_get_reduced_row(long pl, double lon_first, double lon_last, long *npoints, long *ilon_first, long *ilon_last)
{
  ...
  /* computing integer number of points and coordinates without using floating
   * point resolution*/
...
static bool
check_range(long n, double *vals, double valid_min, double valid_max)
{
  bool status = false;
...
bool
grid_has_proj4param(int gridID)
{
...
static char *
grid_get_proj4param(int gridID)
{
      ...
      if (proj4param)
      ...
      else
        {
          ...
          if (projtype == CDI_PROJ_RLL)
            lproj_rll = true;
          else if (projtype == CDI_PROJ_LAEA)
            lproj_laea = true;
          else if (projtype == CDI_PROJ_LCC)
            lproj_lcc = true;
          else if (projtype == CDI_PROJ_SINU)
            lproj_sinu = true;
          else
            {
...
int
gridToUnstructured(int gridID1, int lbounds)
{
  int gridtype = gridInqType(gridID1);
  size_t gridsize = gridInqSize(gridID1);
...
int
gridWeightsOld(int gridID, double *weights)
{
  int status = FALSE;
...
int
gridWeights(int gridID, double *grid_wgts)
{
  int w_status = 1;
  int a_status = 0;
