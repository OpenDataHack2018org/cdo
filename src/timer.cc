...
void
cdoProcessTime(double *utime, double *stime)
{
#if defined(HAVE_SYS_TIMES_H)
  struct tms tbuf;
...
static void
counter_init(counter_t *counter)
{
  counter->cputime = 0;
}

void
counter_start(counter_t *counter)
{
...
void
counter_stop(counter_t *counter)
{
  counter->cputime = get_time_val(counter->mark);
}

double
counter_cputime(counter_t counter)
{
