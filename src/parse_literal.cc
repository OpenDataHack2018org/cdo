...
int
literal_get_datatype(const char *literal)
{
  if (literal && *literal)
    {
      char *endptr;
...
int
literals_find_datatype(int n, char **literals)
{
  int dtype = -1;
...
int
literal_to_int(const char *literal)
{
  int ival = INT_MAX;
...
double
literal_to_double(const char *literal)
{
  double dval = DBL_MAX;
...
{
  const char *literals[]
      = { "127b",    "-32768s",     "-2147483647",      "-1.e+36f",
          "1.e+308", "temperature", "surface pressure", "1000." };
