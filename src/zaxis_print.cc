...
printDblsPrefixAutoBrk(FILE *fp, int dig, const char prefix[], int nbyte0,
                       size_t n, const double vals[], size_t extbreak)
{
  fputs(prefix, fp);
  int nbyte = nbyte0;
...
zaxis_print_kernel(int zaxisID, FILE *fp)
{
  ...
  if (nlevels == 1 && zaxisInqScalar(zaxisID))
    fprintf(fp, "scalar    = true\n");
  ...
  if (nvals)
    {
      ...
      printDblsPrefixAutoBrk(fp, dig, prefix, (int) sizeof(prefix) - 1, nvals,
                             vals, 0);
    }
