...
/* extern int SetMagicsParameterValue( const char *param_name, const char
 * *param_type, const char *param_value ) */
...
/* int GetMagicsParameterInfo( const char *user_name, char **magics_name, char
 * **magics_type ) */

int
GetMagicsParameterInfo(const char *user_name, char *param_value)
{
  static int once = 1;
  int ret_flag = 0;
  ...
  if (result)
    ...
    {
      /* Call the Reset functions of all the features to Reset the magics params
       * to default in the calling function */
      ret_flag = 1;
    }
