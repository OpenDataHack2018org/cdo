...
int
my_progress_func(void *stdout_is_tty, double t, /* dltotal */
                 double d,                      /* dlnow */
...
ftpget(int flag, const char *url, const char *path, const char *target, const char *source)
{
  ...
  struct FtpFile ftpfile = { NULL, /* name to store the file as if succesful */
                             NULL };
  char filename[8192];
...
int
ftprmd(const char *url, const char *path)
{
  int status = 0;
#ifdef HAVE_LIBCURL
  CURL *curl;
...
void
exp_run(int argc, char *argv[], const char *cdoExpName)
{
  ...
  */
  ...
  if (envstr)
    {
      if (cdoVerbose) fprintf(stderr, "CDO_REMOTE_PATH        = %s\n", envstr);
  ...
  if (envstr)
    {
      if (cdoVerbose) fprintf(stderr, "CDO_REMOTE_TMP         = %s\n", envstr);
  ...
  if (envstr)
    {
      if (cdoVerbose) fprintf(stderr, "CDO_REMOTE_FTP         = %s\n", envstr);
  ...
  */
  ...
  fprintf(jobfilep, "#setenv LD_LIBRARY_PATH "
                    "/opt/gridware/sge/lib/${SGE_ARCH}:$LD_LIBRARY_PATH\n");
  fprintf(jobfilep, "%s\n", commandline);
