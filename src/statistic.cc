  ...
  for (l = 0; l < n; l++)
    {
      ...
      while (1)
        {
          for (m = l; m < n - 1; m++)
            if (fabs(e[m]) <= eps * (fabs(d[m]) + fabs(d[m + 1]))) break;
          if (m == l)
            {
...
solution_of_linear_equation(double **a, double *b, int n)
{
  ...
  if (not_singular) lu_backsubstitution(a, n, index, b);
  ...
  while (TRUE)
    {
      ...
      b2 = p + chi_square(n + 2, *c1, prompt) - chi_square(n + 2, *c2, prompt);
      /* Solve ((a11,a12),(a21,a22))*(delta_c1,delta_c2)==(b1,b2) */
      det = a11 * a22 - a12 * a21;
      delta_c1 = (b1 * a22 - b2 * a12) / det;
      delta_c2 = (b2 * a11 - b1 * a21) / det;
      if (fabs(delta_c1) < *c1 * eps && fabs(delta_c2) < *c2 * eps) break;
      if (*c1 + delta_c1 >= n)
        *c1 = (n + *c1) / 2;
...
/* ********************************************************************************
 */
/* This routine rotates columns/rows i and j of a symmetric Matrix M in a
 * fashion,  */
