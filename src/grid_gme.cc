...
void
factorni(int kni, int *kni2, int *kni3)
{
  /**********************************************************************/
...
circum_center(struct cart *v0, struct cart *v1, struct cart *v2)
{
  ...
  cnorm = sqrt(cu.x[0] * cu.x[0] + cu.x[1] * cu.x[1] + cu.x[2] * cu.x[2]);
...
void
gme_grid_restore(double *p, int ni, int nd)
{
  int j;
...
int main(int argc, char *argv[])
{
  ...
  if (( poly  = (struct polygon*) Malloc((ni+1)*(ni+1)*nd*sizeof(struct
polygon))) == NULL) { perror("malloc poly"); exit (-1);
  }
        ...
        for (j1 = 0; j1 <= ni; j1++) {
          ...
          if (mask[j1+id1*j2+id2*jd+ioffset]) {
            ...
            for (jm = 1; jm <= poly[j1+id1*j2+id2*jd+ioffset].type; jm++) {
              ...
              area[j1+id1*j2+id2*jd+ioffset] = area[j1+id1*j2+id2*jd+ioffset] +
areas(&c1, &c2, &c3);
            }
