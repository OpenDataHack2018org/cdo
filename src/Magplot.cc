...
const char *PROJECTION_TABLE[] = { "cylindrical",
                                   "polar_stereographic",
                                   "polar_north",
                                   "geos",
                                   "meteosat",
...
static void
VerifyPlotParameters(int num_param, char **param_names, int opID)
{
  int i, j, k;
  int found = FALSE, syntax = TRUE, halt_flag = FALSE,
      /* file_found = TRUE, */ split_str_count;
  int param_count = 0;
  ...
  /*
    char  *contour_params[] =
    {"ymin","ymax","count","interval","list","colour","thickness","style"}; char
    *shaded_params[]  =
    {"ymin","ymax","count","interval","list","colour_min","colour_max","colour_table","step_freq"};
    char  *grfill_params[]  =
    {"ymin","ymax","count","interval","list","colour_min","colour_max","colour_table","resolution"};
  */
                      ...
                      if (!split_str_count)
                      ...
                      else
                        {
                          for (k = 0; k < split_str_count; k++)
                            {
                              if (!IsNumeric(split_str1[k])) syntax = FALSE;
                            }
                          if (syntax == TRUE)
                            {
  ...
  if (isRGB)
    {
      if (strchr(colour_in, ';') == NULL || strstr(colour_in, "RGB(") == NULL)
        {
          cdoWarning("Found 'RGB=true',Specify Colour in 'RGB(r;g;b)' ( where "
                     "r,g,b in [0.0,1.0] ) format!");
          Free(split_str);
          return 1;
        }
  ...
  else
    {
      if (strchr(colour_in, ';') != NULL || strstr(colour_in, "RGB(") != NULL)
        {
          cdoWarning("Found Colour with 'RGB(r;g;b)' format, set parameter "
                     "RGB='true' !");
          Free(split_str);
          return 1;
        }
      ...
      cdoWarning("Specified Colour not in Standard colour list, resetting to "
                 "blue(default colour)!");
      return 1;
    }

  if (DBG) cdoWarning("Colour %s verified!", ref);
...
checkstyle(char *style_in)
{
  ...
  if (!found) cdoWarning(" Style specified with Improper value!");
...
checkdevice(char *device_in)
{
  ...
  if (!found) cdoWarning(" Device specified with Improper value!");
  ...
  if (!found)
    {
      ...
      cdoWarning(" cylindrical polar_stereographic polar_north geos meteosat "
...
Magplot(void *process)
{
  ...
  */
  ...
  while ((nrecs = cdoStreamInqTimestep(streamID, tsID)))
    {
      if (ANIM_FLAG)
        {
          if (nrecs > 1)
            {
              cdoWarning("File has more than one variable! Animation creation "
                         "not possible!!!");
              break;
            }
          if (tsID % STEP_FREQ)
      ...
      else
        {
          if (STEP_FREQ)
          ...
          else
            {
              if (tsID)
                {
                  cdoWarning("File variables have values at more than one time "
                             "step! Images created for first time step!!!");
                  cdoWarning("To plot steps at a particular interval, set "
                             "'step_freq' to the frequency of the steps to be "
                             "plotted!!!");
                  cdoWarning("To plot steps at random interval, set "
                             "'step_freq' to '1' and select the steps using "
      ...
      if (DBG) fprintf(stderr, "Date %s Time %s\n", vdatestr, vtimestr);
      ...
      if( !STEP_FREQ  && tsID )
        {
           cdoWarning("File variables have values at more than one time step!
      Images created for first time step!!!"); cdoWarning("To plot steps at a
      particular interval, set 'step_freq' to the frequency of the steps to be
      plotted!!!"); cdoWarning("To plot steps at random interval, set
      'step_freq' to '1' and select the steps using the selection
      operators!!!"); break;
        }
      else
        {
  ...
  if (ANIM_FLAG)
    {
      if (FILE_SPLIT == TRUE) cdoWarning("File split parameter ignored!!!");
    }
  quit_MAGICS();
