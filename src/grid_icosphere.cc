...
static inline
double rad2deg(double v) { return v*180./M_PI; }

// Converts cartesian coordinates to geographical
static inline
void cc2gc(double xyz[], double *lon, double *lat)
{
...
static inline
vec3 addVector(const vec3 &a, const vec3 &b)
{
  vec3 c;
...
static inline
vec3 subVector(const vec3 &a, const vec3 &b)
{
  vec3 c;
  for ( unsigned i = 0; i < 3; ++i ) c[i] = a[i] - b[i];
  return c;
...
static inline
vec3 normalizeVector(const vec3 &a)
{
  vec3 c;
...
static inline
void d_normalize(vec3 &v)
{
  double dnorm = sqrt(dotProduct(v,v));
  for ( unsigned i = 0; i < 3; ++i ) v[i] /= dnorm;
}

vec3 circumCenterMean(const vec3 &v0, const vec3 &v1, const vec3 &v2)
{
  /*
    v0, v1, v2: the coordinates of the three triangle vertices (unit vectors) in counter clockwise order
    center: the coordinates of the circumcenter unless co-linear
  */
  ...
  if ( dotProduct(cu0, v0) < 0.0 ) for ( unsigned i = 0; i < 3; ++i ) cu0[i] = -cu0[i];
  d_normalize(cu0);

  e1 = subVector(v2, v1);
  e2 = subVector(v0, v1);
  vec3 cu1 = vectorProduct(e1, e2);
  if (dotProduct(cu1, v1) < 0.0 ) for ( unsigned i = 0; i < 3; ++i ) cu1[i] = -cu1[i];
  d_normalize(cu1);

  e1 = subVector(v0, v2);
  e2 = subVector(v1, v2);
  vec3 cu2 = vectorProduct(e1, e2);
  if ( dotProduct(cu2, v2) < 0.0 ) for ( unsigned i = 0; i < 3; ++i ) cu2[i] = -cu2[i];
  d_normalize(cu2);
