...
void
after_gp2sp(struct Control *globs, struct Variable *vars, int ccode)
{
...
void
after_FCrh2FCsh(struct Control *globs, struct Variable *vars)
{
...
void
after_SPuv2SPdv(struct Control *globs, struct Variable *vars)
{
...
void
after_FCsh2FCrh(struct Control *globs, struct Variable *vars)
{
  const long fieldSize = globs->DimGP * globs->NumLevelRequest;
...
void
after_processPL(struct Control *globs, struct Variable *vars)
{
  int code, l;
  long fieldSize;
  int lindex, nlevel;
  ...
  if (vars[NET_HEAT].comp)
    {
      ...
      if ( Source == S_ECHAM5 )
        {
          MultVectorScalar(vars[NET_HEAT].hybrid, vars[218].hybrid, (-3.345e5),
      globs->DimGP, vars[218].nmiss, vars[218].missval);
          Add2Vectors(vars[NET_HEAT].hybrid, vars[NET_HEAT].hybrid,
      vars[176].hybrid, globs->DimGP); Add2Vectors(vars[NET_HEAT].hybrid,
      vars[NET_HEAT].hybrid, vars[177].hybrid, globs->DimGP);
          Add2Vectors(vars[NET_HEAT].hybrid, vars[NET_HEAT].hybrid,
      vars[146].hybrid, globs->DimGP); Add2Vectors(vars[NET_HEAT].hybrid,
      vars[NET_HEAT].hybrid, vars[147].hybrid, globs->DimGP);
          Add2Vectors(vars[NET_HEAT].hybrid, vars[NET_HEAT].hybrid,
      vars[206].hybrid, globs->DimGP); Sub2Vectors(vars[NET_HEAT].hybrid,
      vars[NET_HEAT].hybrid, vars[208].hybrid, globs->DimGP);
          Sub2Vectors(vars[NET_HEAT].hybrid, vars[NET_HEAT].hybrid,
      vars[209].hybrid, globs->DimGP);
        }
      else
      */
      {
...
void
after_processML(struct Control *globs, struct Variable *vars)
{
  int code, l, i;
  long fieldSize = 0;
  int lindex, nlevel;
...
after_legini_setup(struct Control *globs, struct Variable *vars)
{
  ...
  int flag = 1;
  // if ( globs->poli && globs->pold && globs->pdev == NULL && globs->pol2 ==
