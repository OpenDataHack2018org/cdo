...
percentile(double *array, size_t len, double pn)
{
  ...
  if (percentile_method == NRANK)
    percentil = percentile_nrank(array, len, pn);
  else if (percentile_method == NIST)
    percentil = percentile_nist(array, len, pn);
...
percentile_set_method(const char *methodstr)
{
  ...
  if (strcmp("nrank", methodname) == 0)
    percentile_method = NRANK;
  else if (strcmp("nist", methodname) == 0)
    percentile_method = NIST;
  else if (strcmp("numpy", methodname) == 0)
    percentile_method = NUMPY;
  else if (strcmp("numpy_linear", methodname) == 0)
    {
      percentile_method = NUMPY;
      interpolation_method = LINEAR;
    }
  else if (strcmp("numpy_lower", methodname) == 0)
    {
      percentile_method = NUMPY;
      interpolation_method = LOWER;
    }
  else if (strcmp("numpy_higher", methodname) == 0)
    {
      percentile_method = NUMPY;
      interpolation_method = HIGHER;
    }
  else if (strcmp("numpy_nearest", methodname) == 0)
    {
      percentile_method = NUMPY;
      interpolation_method = NEAREST;
    }
  else
    cdoAbort("Percentile method %s not available!", methodstr);
}
