...
Gradsdes(void *process)
{
  ...
  if (filetype != CDI_FILETYPE_GRB && nvars != nvarsout) cdoAbort("Too many different grids!");
      ...
      if (idd0 > 28)
        {
          /* int iddx = idd0; */
          idd0 = 1;
          cdoPrint("Reset start date to %02d:%02dZ%02d%s%04d", ihh0, imn0, idd0, cmons[imm0 - 1], iyy0);
        }
    }
