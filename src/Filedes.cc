...
static void
printHistory(FILE *fp, int streamID)
{
...
static void
partab(FILE *fp, int streamID, int option)
{
  ...
  for (varID = 0; varID < nvars; varID++)
    {
      ...
      if (option == 2) fprintf(fp, "  missing_value=%g\n", missval);
          ...
          if (varlongname[0])
            {
              ...
              if (varunits[0]) fprintf(stdout, " [%s]", varunits);
            }
          fprintf(stdout, "\n");
        }
    }
