...
void
cmor_check_init(int nvars, var_t *vars)
{
...
void
cmor_check_eval(int vlistID, int nvars, var_t *vars)
{
...
void
cmor_check_prep(var_t *var, long gridsize, double missval, double *array)
{
  if (var->checkvalid || var->check_min_mean_abs || var->check_max_mean_abs)
    {
      double aval;
...
static void
apply_cmorlist(list_t *pmlist, int nvars, int vlistID2, var_t *vars)
{
          ...
          for (listNode_t *kvnode = kvlist->head; kvnode; kvnode = kvnode->next)
            {
              ...
              if (STR_IS_EQ(key, "standard_name"))
                ...
                cdiDefAttTxt(vlistID2, varID, key, (int) strlen(value), value);
              else if (STR_IS_EQ(key, "cell_measures"))
                cdiDefAttTxt(vlistID2, varID, key, (int) strlen(value), value);
