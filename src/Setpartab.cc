          ...
          for (listNode_t *kvnode = kvlist->head; kvnode; kvnode = kvnode->next)
            {
              ...
              if (lv1 && STR_IS_EQ(key, "standard_name"))
                vlistDefVarStdname(vlistID2, varID, value);
              else if (lv1 && STR_IS_EQ(key, "long_name"))
                vlistDefVarLongname(vlistID2, varID, value);
              else if (lv1 && STR_IS_EQ(key, "units"))
                ...
                ;
              else if (lv1 && STR_IS_EQ(key, "cell_methods"))
                cdiDefAttTxt(vlistID2, varID, key, (int) strlen(value), value);
              else if (lv1 && STR_IS_EQ(key, "cell_measures"))
                cdiDefAttTxt(vlistID2, varID, key, (int) strlen(value), value);
              else if (lv1 && STR_IS_EQ(key, "delete"))
                var->remove = parameter2bool(value);
              else if (lv1 && STR_IS_EQ(key, "convert"))
                var->convert = parameter2bool(value);
...
Setpartab(void *process)
{
  ...
  if (operatorID == SETCODETAB)
    ptmode = CODE_NUMBER;
  else if (operatorID == SETPARTABC)
    ptmode = CODE_NUMBER;
  else if (operatorID == SETPARTABP)
    ptmode = PARAMETER_ID;
  else if (operatorID == SETPARTABN)
    ptmode = VARIABLE_NAME;
