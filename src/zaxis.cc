...
static void
zaxis_read_data(size_t nkv, kvmap_t *kvmap, zaxis_t *zaxis, size_t *iatt,
                const char *dname)
{
  // char uuidStr[256];
  ...
  for (size_t ik = 0; ik < nkv; ++ik)
    {
      ...
      // printf("%s = ", key); if ( value  ) printf("%s", kv->value);
      // printf("\n");
      ...
      if (STR_IS_EQ(key, "zaxistype"))
        {
          ...
          if (STR_IS_EQ(zaxistype, "pressure"))
            zaxis->type = ZAXIS_PRESSURE;
          else if (STR_IS_EQ(zaxistype, "hybrid_half"))
            zaxis->type = ZAXIS_HYBRID_HALF;
          else if (STR_IS_EQ(zaxistype, "hybrid"))
            zaxis->type = ZAXIS_HYBRID;
          else if (STR_IS_EQ(zaxistype, "height"))
            zaxis->type = ZAXIS_HEIGHT;
          else if (STR_IS_EQ(zaxistype, "depth_below_sea"))
            zaxis->type = ZAXIS_DEPTH_BELOW_SEA;
          else if (STR_IS_EQ(zaxistype, "depth_below_land"))
            zaxis->type = ZAXIS_DEPTH_BELOW_LAND;
          else if (STR_IS_EQ(zaxistype, "isentropic"))
            zaxis->type = ZAXIS_ISENTROPIC;
          else if (STR_IS_EQ(zaxistype, "surface"))
            zaxis->type = ZAXIS_SURFACE;
          else if (STR_IS_EQ(zaxistype, "generic"))
            zaxis->type = ZAXIS_GENERIC;
          else
            cdoAbort("Invalid zaxis type: %s (zaxis description file: %s)",
                     zaxistype, dname);
        }
      else if (STR_IS_EQ(key, "datatype"))
        {
          ...
          if (STR_IS_EQ(datatype, "double"))
            zaxis->datatype = CDI_DATATYPE_FLT64;
          else if (STR_IS_EQ(datatype, "float"))
            zaxis->datatype = CDI_DATATYPE_FLT32;
          else
            cdoAbort("Invalid datatype: %s (zaxis description file: %s)",
                     datatype, dname);
        }
      else if (STR_IS_EQ(key, "size"))
        zaxis->size = parameter2int(value);
      else if (STR_IS_EQ(key, "scalar"))
        zaxis->scalar = parameter2bool(value);
      else if (STR_IS_EQ(key, "vctsize"))
        zaxis->vctsize = parameter2int(value);
      else if (STR_IS_EQ(key, "name"))
        strcpy(zaxis->name, parameter2word(value));
      else if (STR_IS_EQ(key, "units"))
        strcpy(zaxis->units, parameter2word(value));
      else if (STR_IS_EQ(key, "longname"))
        strcpy(zaxis->longname, value);
      else if (STR_IS_EQ(key, "levels"))
        {
          if (zaxis->size == 0)
            cdoAbort("size undefined (zaxis description file: %s)!", dname);
          if (zaxis->size != nvalues)
            cdoAbort("size=%zu and number of levels=%zu differ!", zaxis->size,
                     nvalues);
      ...
      else if (STR_IS_EQ(key, "lbounds"))
        {
          if (zaxis->size == 0)
            cdoAbort("size undefined (zaxis description file: %s)!", dname);
          if (zaxis->size != nvalues)
            cdoAbort("size=%zu and number of lbounds=%zu differ!", zaxis->size,
                     nvalues);
      ...
      else if (STR_IS_EQ(key, "ubounds"))
        {
          if (zaxis->size == 0)
            cdoAbort("size undefined (zaxis description file: %s)!", dname);
          if (zaxis->size != nvalues)
            cdoAbort("size=%zu and number of ubounds=%zu differ!", zaxis->size,
                     nvalues);
...
zaxis_read_attributes(size_t iatt, size_t nkv, kvmap_t *kvmap, int zaxisID)
{
  const char *reserved_keys[]
      = { "zaxistype", "size",   "scalar",  "vctsize", "name", "units",
      ...
      else
        {
          for (int n = 0; n < num_rkeys; ++n)
            if (STR_IS_EQ(key, reserved_keys[n]))
              cdoAbort("Found reserved key word >%s< in attribute names! Check "
                       "name or position of >%s<.",
                       key, attkey0);
        }
  ...
  for (listNode_t *kvnode = kvlist->head; kvnode; kvnode = kvnode->next)
    {
      ...
      if (ik == 0 && !STR_IS_EQ(kv->key, "zaxistype"))
        cdoAbort(
            "First zaxis description key word must be >zaxistype< (found: %s)!",
            kv->key);

      if (kv->nvalues == 0)
        {
          cdoWarning("Z-axis description key word %s has no values, skipped!",
                     kv->key);
        }
      else
        {
...
cdoDefineZaxis(const char *zaxisfile)
{
  ...
  if (zaxisID == CDI_UNDEFID)
    cdoAbort("Invalid zaxis description file %s!", zaxisfile);
