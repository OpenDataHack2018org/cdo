  ...
  for (int ilev = 0; ilev < nlev2; ++ilev)
    {
      ...
      // if ( cdoVerbose ) cdoPrint("level %d: idx1=%d idx2=%d wgt1=%g wgt2=%g",
      // ilev, idx1, idx2, wgt1, wgt2);
      ...
      for (size_t i = 0; i < gridsize; i++)
        {
          ...
          /* if (cdoVerbose) printf("i:%d level %d: idx1=%d idx2=%d
           * (offset+i:%d) wgt1=%g wgt2=%g var1L1:%g var1L2:%g ", i,       ilev,
           * idx1,   idx2,    offset+i,    wgt1,   wgt2,
           * var1L1,   var1L2);
           */
  ...
  for (int i2 = 0; i2 < nlev2; ++i2)
    {
      ...
      if (i1 - 1 == 0)  // destination levels ios not covert by the first two
                        // input z levels
        {
          lev_idx1[i2] = 1;
          lev_idx2[i2] = 1;
      ...
      else if (i1 == nlev1 - 1)  // destination level is beyond the last value
                                 // of the input z field
        {
          lev_idx1[i2] = nlev1 - 2;
          lev_idx2[i2] = nlev1 - 2;
      ...
      // backshift of the indices because of the two additional levels in input
      // vertical coordinate
      lev_idx1[i2]--;
      lev_idx2[i2]--;
      /*
...
bool
levelDirUp(int nlev, double *lev)
{
...
bool
levelDirDown(int nlev, double *lev)
{
...
void
vert_init_level_0_and_N(int nlev, size_t gridsize, double *zlevels)
{
  /*
  ...
  /* Add artificial values for indication of extrapolation areas (lowermost +
   * upmost levels) */
  if (lup)
    {
      for (size_t i = 0; i < gridsize; i++)
